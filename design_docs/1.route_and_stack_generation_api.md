...
The two question are intertwined depending on how we approach it:
  ...
  3. Either '/' must be defined and only sometimes part of the stack
...
```
  home: '/dashboard'
```
3. can be implemented By replacing the current route map by a `Route(path, builder)` class

```
  Route(path: '/products', builder: ...),
  Route(path: '/', builder: ..., matchChildren: true / false ); 
```

If matchChildren is true, it is part of the stack, if not then it's only part of the upstack when it's an exact match. This could apply to other routes as well so this seems like a good solution.


*What happens if the user doesn't specify '/' route ?*

We could say that the '/' route **must** be defined

or we could have option 1. together with 3.

```
  // here the dashboard page will be the uppermost route on the /dashboard/* path
  // and accessing / redirects to /dashboard
  Route(path: '/', redirectTo: '/dashboard' ); 
  // or
  // here the dashboard page will be the uppermost route on /* page
  Route(path: '/', builder: () => DashboardPage()) ); 
```

Although this would give more freedom to the user this seems error prone and doesn't seem to fit most cases
of applications. Most applications have authentication. Having an home route that can potentially be on the stack of any route if the user doesn't pay attention, because it matches any child might bring more confusion than the alternative of just *always* redirecting the home route to another.
